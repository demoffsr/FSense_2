...
- `CHAT_CONVERSATIONAL_ROUTER` - defaults to `true`; set `false` to fall back to IntentClassifierAgent + QuickReplyAgent pattern

## Current Status

**Version 0.7.0** - Conversational Chat Experience: Replaced `IntentClassifierAgent` + `QuickReplyAgent` two-step pattern in General chat mode with a single `ConversationRouter` that enables natural multi-turn conversations. Greetings get friendly responses instead of canned "I'm a flower assistant" text. Ambiguous messages trigger clarifying questions instead of immediately running the 10-agent pipeline. When enough context is gathered (relationship/occasion), the router signals "recommend" with extracted context that enriches the pipeline via `UserPriors`. New endpoint `/api/chat/route`. Backend `run_flower_chat()` extended with `relationship_hint` and `occasion_hint`. `/api/chat` extended with `pre_routed`, `relationship_hint`, `occasion_hint` to skip double-classification. iOS General mode uses `routeMessage()` → branch on action. Heuristic fast-paths for greetings, flower requests, off-topic, clarifications (no AI call). AI routing via GPT-4o-mini for ambiguous messages. Rollback via `CHAT_CONVERSATIONAL_ROUTER=false`. Tests in `backend/tests/test_conversation_router.py` (65 tests).
